...
function players() {
    ...
    $.getJSON('/api/minecraft/players', function(data) {
        if (data.players.now == 0){
            var message = "Aucun Joueur"
        }
        if (data.players.now == 1){
            var message = data.players.now + ' ' + "Joueur"
        }
        if (data.players.now > 1){
            var message = data.players.now + ' ' + "Joueurs"
        }
        ...
        console.log('%c' + data.players.now + " connecté (s) !", 'background: #222; font-size: 2em; color: #9b59b6');
