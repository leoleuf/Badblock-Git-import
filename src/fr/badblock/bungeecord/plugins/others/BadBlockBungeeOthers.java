						...
						try {
							if (resultSet.next()) {
								...
								try {
									...
									try {
										BadblockDatabase.getInstance().addSyncRequest(new Request("SELECT slots FROM absorbances WHERE `bungeeTimestamp` > " + System.currentTimeMillis() + ";", RequestType.GETTER) {
											@Override
