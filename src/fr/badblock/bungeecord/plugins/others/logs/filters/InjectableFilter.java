...
/**
 * A logger-specific log filter that can be injected into and removed from its
 * logger.
 *
...
public interface InjectableFilter extends Filter {
	...
	/**
	 * Checks whether this filter is currently injected into the logger. The filter
	 * is not injected if the logger's filter has ben changed somewhere else.
	 *
	 ...
	 * use a wrapper filter for compatibility with filters already added to the
	 * logger.
	 *
	...
	/**
	 * Attempts to reset the logger's filter to its state before this filter was
	 * injected. The behaviour of this method is undefined if that logger's current
	 * filter was not injected by this instance's {@link #inject() inject method},
	 * and entirely implementation-dependent.
	 *
