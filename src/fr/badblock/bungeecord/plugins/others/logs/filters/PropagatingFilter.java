...
/**
 * An abstract base class for injectable filters that also call previously
 * installed log filters, therefore providing more compatibility. This
 * implementation does propagate calls to previously installed filters.
 *
...
public abstract class PropagatingFilter extends AbstractInjectableFilter {
	...
	public Filter inject() {
		...
		if (isInjected()) { // we're injected or in the compound
			...
			getLogger().setFilter(new CompoundFilter( // create compound
					getLogger(), Arrays.asList(this, previousFilter) // called in that order
			));
	...
	public boolean reset() {
		if (inCompound()) { // we're in the compound
			...
			getLogger().log(Level.WARNING, "[QuietCord] Could not reset filter {0} because no longer in compound!",
