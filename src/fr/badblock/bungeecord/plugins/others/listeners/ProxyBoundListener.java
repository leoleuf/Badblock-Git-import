...
public class ProxyBoundListener implements Listener {
	...
	public void onProxyBound(ProxyBoundEvent event) {
					...
					public void done(ResultSet resultSet) {
						try {
							if (resultSet.next()) {
								...
								if (recordId != 0) {
									DNSDeleteRecord dns = new DNSDeleteRecord(bungeeOthers.getAccess(), "badblock.fr",
											recordId);
									try {
										...
										if (jsonObject != null) {
											...
											BadblockDatabase.getInstance().addSyncRequest(new Request(
													"INSERT INTO history(ip, action, log, date) VALUES('" + a
															+ "', 'Deleted record in cloudflare (ProxyBoundEvent1 / ID "
															+ recordId + ")', '" + jsonObject.toString() + "', '"
											...
											BadblockDatabase.getInstance().addSyncRequest(new Request(
													"INSERT INTO history(ip, action, log, date) VALUES('" + a
															+ "', 'Fail: Deleted record in cloudflare (ProxyBoundEvent1 / ID "
															+ recordId + ")', 'null', '"
															+ new SimpleDateFormat("dd/MM/yyyy HH:mm:ss.SS")
																	.format(new Date())
															+ "')",
		...
		@SuppressWarnings("deprecation")
		DNSAddRecord dns = new DNSAddRecord(bungeeOthers.getAccess(), "badblock.fr", RecordType.IPV4Address,
				"roundrobin." + (LadderBungee.getInstance().countEnvironment == 1 ? "eu" : "na") + ".badblock.fr",
				ProxyServer.getInstance().getConfig().getListeners().iterator().next().getHost().getHostString(),
				new TimeUnit(UnitType.MINUTES, 2));
		try {
			...
			if (object != null) {
								...
								if (obj.has("rec_id")) {
									...
									BadblockDatabase.getInstance()
											.addSyncRequest(
													new Request(
															"INSERT INTO history(ip, action, log, date) VALUES('" + a
																	+ "', 'Added record in cloudflare (ID "
																	+ bungeeOthers.getRecordId() + ")', '"
																	+ object.toString() + "', '"
																	+ new SimpleDateFormat("dd/MM/yyyy HH:mm:ss.SS")
																			.format(new Date())
																	+ "')",
															RequestType.SETTER));
									BadblockDatabase.getInstance()
											.addSyncRequest(new Request("UPDATE absorbances SET recordId = '"
													+ bungeeOthers.getRecordId() + "' WHERE ip = '" + a + "'",
													RequestType.SETTER));
								}
							}
						}
				...
				System.out.println("/!\\ BUNGEEDNS<ADDED/" + LadderBungee.getInstance().bungeePlayerCount + "/"
						+ BadBlockBungeeOthers.getInstance().getConnections() + "> /!\\");
			} else
				System.out.println("/!\\ BUNGEEDNS<NOT-ADDED/" + LadderBungee.getInstance().bungeePlayerCount + "/"
						+ BadBlockBungeeOthers.getInstance().getConnections() + "> /!\\");
			dns.close();
		} catch (Exception e) {
			e.printStackTrace();
								...
								if (recordId != 0) {
									...
									try {
										...
										if (jsonObject != null) {
											BadblockDatabase.getInstance().addSyncRequest(new Request(
													"INSERT INTO history(ip, action, log, date) VALUES('" + a
															+ "', 'Deleted record in cloudflare (ProxyUnableToBindEvent / ID "
															+ recordId + ")', '" + jsonObject.toString() + "', '"
															+ new SimpleDateFormat("dd/MM/yyyy HH:mm:ss.SS")
																	.format(new Date())
															+ "')",
											...
											BadblockDatabase.getInstance().addSyncRequest(new Request(
													"INSERT INTO history(ip, action, log, date) VALUES('" + a
															+ "', 'Fail: Deleted record in cloudflare (ProxyUnableToBindEvent / ID "
															+ recordId + ")', 'null', '"
															+ new SimpleDateFormat("dd/MM/yyyy HH:mm:ss.SS")
																	.format(new Date())
															+ "')",
