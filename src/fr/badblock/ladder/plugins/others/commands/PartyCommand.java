...
import fr.badblock.ladder.plugins.others.friends.Party;
...
public class PartyCommand extends Command {
	...
	public void executeCommand(CommandSender sender, String[] args) {
		...
		if (args[0].equalsIgnoreCase("invite") || args[0].equalsIgnoreCase("inviter") || args[0].equalsIgnoreCase("i")
				|| args[0].equalsIgnoreCase("a") || args[0].equalsIgnoreCase("add")) {
			invite(player, args);
			return;
		}
		...
		if (args[0].equalsIgnoreCase("set") || args[0].equalsIgnoreCase("state")
				|| args[0].equalsIgnoreCase("toggle")) {
			set(player, args);
			return;
		}
		if (args[0].equalsIgnoreCase("leader") || args[0].equalsIgnoreCase("admin")
				|| args[0].equalsIgnoreCase("lead")) {
			admin(player, args);
			return;
		}
		if (args[0].equalsIgnoreCase("follow") || args[0].equalsIgnoreCase("suivi") || args[0].equalsIgnoreCase("f")) {
			follow(player, args);
			return;
		}
		if (args[0].equalsIgnoreCase("quit") || args[0].equalsIgnoreCase("kit") || args[0].equalsIgnoreCase("quitter")
				|| args[0].equalsIgnoreCase("leave")) {
			leave(player, args);
			return;
		}
		if (args[0].equalsIgnoreCase("follow") || args[0].equalsIgnoreCase("suivi")) {
			follow(player, args);
			return;
		}
		...
		if (args[0].equalsIgnoreCase("teleport") || args[0].equalsIgnoreCase("tp")
				|| args[0].equalsIgnoreCase("teleporter") || args[0].equalsIgnoreCase("suivre")
				|| args[0].equalsIgnoreCase("tpa")) {
			tp(player, args);
			return;
		}
		if (args[0].equalsIgnoreCase("infos") && sender.hasPermission("party.infos")) {
			infos(player);
			return;
		}
		player.sendMessage(I18N.getTranslatedMessage("party.unknowncommand"));
	}
	...
	public void list(Player player) {
		...
		list(player, fp.getParty());
	}

	public static void list(CommandSender sender, Party party) {
		if (party == null) {
		...
		new Thread() {
			...
			public void run() {
				for (String string : strings) {
					if (string.equals("@0")) {
						sender.sendMessages(I18N.getTranslatedMessages("commands.party.list.leader", party.getLeader(),
	...
	public void invite(Player player, String[] args) {
		...
		// default party slots
		Integer partySlots = player.getPermissionValue("partySlots", Integer.class);
		if (partySlots == null) partySlots = 4;
		if (fp.party != null && fp.party.getPlayers().size() + fp.party.getWaitingPlayers().size() >= partySlots) {
			player.sendMessage(I18N.getTranslatedMessage("party.maxslots", partySlots));
			return;
		}
	...
	public void accept(Player player, String[] args) {
		...
		if (to.groupFollow) {
			...
			toPlayer.sendToBungee("playersWithHim"); player.sendToBukkit("playersWithHim");
		}
		fp.party.sendToBeChangedInSQL();
		this.updateFollow(fp, player);
		Integer partySlots = toPlayer.getPermissionValue("partySlots", Integer.class);
		if (partySlots == null) partySlots = 4;
		for (String pl : fp.party.getPlayers())
			if (isOnline(pl))
				Ladder.getInstance().getPlayer(pl).sendMessage(
						I18N.getTranslatedMessage("party.accept.join", player.getName(), fp.party.getPlayers().size(), partySlots));
	}
	...
	public void kick(Player player, String[] args) {
		...
		for (String pl : fp.party.getPlayers()) {
			...
			if (plo != null) {
				plo.sendMessage(I18N.getTranslatedMessage("party.kick.message", pseudo, fp.party.getPlayers().size(), partySlots));
			}
		}
		if (fp.groupFollow) {
	...
	public void admin(Player player, String[] args) {
		...
		if (isOnline(pseudo)) {
			...
			if (po != null) {
				...
				po.sendToBungee("playersWithHim"); po.sendToBukkit("playersWithHim");
			}
		}
		if (fp.groupFollow) {
			player.setPlayersWithHim(new ArrayList<UUID>());
		...
		if (fp.party.getLeader().equalsIgnoreCase(player.getName())) {
			for (String pl : fp.party.getPlayers())
				if (isOnline(pl)) {
					...
					Integer partySlots = player.getPermissionValue("partySlots", Integer.class);
					if (partySlots == null) partySlots = 4;
					lo.sendMessage(I18N.getTranslatedMessage("party.leave.messageremoved", player.getName(), partySlots));
				} else
					BadblockDatabase.getInstance()
					.addRequest(new Request(
							"UPDATE friends SET party = 'NULL' WHERE pseudo = '"
			...
			player.canJoinHimself(true);
			if (fp.party != null) {
				if (fp.party.getPlayers() != null)
					fp.party.getPlayers().clear();
				if (fp.party.getWaitingPlayers() != null)
					fp.party.getWaitingPlayers().clear();
			}
			if (fp.party != null)
				fp.party.sendToBeChangedInSQL();
			fp.party = null;
			...
			if (fp.party != null && fp.party.getPlayers() != null) {
				fp.party.getPlayers().remove(player.getName());
				fp.party.getPlayers().remove(player.getName().toLowerCase());
			}
			Iterator<String> players = fp.party.getPlayers().iterator();
			while (players.hasNext()) {
				String string = players.next();
			...
			.addRequest(new Request(
					...
					.getInstance().mysql_real_escape_string(player.getName()) + "'",
					RequestType.SETTER));
			...
			if (pp != null) {
				...
				if (pfp != null) {
					...
					if (pfp.party != null && pfp.groupFollow) {
						...
						pp.sendToBungee("playersWithHim"); pp.sendToBukkit("playersWithHim");
					}
				}
			}
			if (fp.party.getPlayersWithoutLeader().size() == 0) {
	...
	public static String getServer(String pseudo) {
		if (!isOnline(pseudo))
	...
	public static String getBungeeServer(String pseudo) {
		if (!isOnline(pseudo))
			return I18N.getTranslatedMessage("party.unknown");
		Player player = Ladder.getInstance().getPlayer(pseudo);
		if (player != null)
	...
	public static boolean isOnline(String pseudo) {
		Player player = Ladder.getInstance().getPlayer(pseudo);
	...
	public static FriendPlayer get(String pseudo) {
		Player player = Ladder.getInstance().getPlayer(pseudo);
		if (player == null)
			return null;
	...
	public static boolean hasFollow(String pseudo) {
	...
	public static AcceptType getType(String pseudo) {
