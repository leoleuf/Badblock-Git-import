...
public class CommandUnmute extends SanctionCommand {
	...
	public void run(CommandSender sender, String[] args) {
		...
		if (!offlinePlayer.getAsPunished().isMute() && !offlinePlayer.getIpAsPunished().isMute()) {
			sender.sendMessage("§c➤ " + args[0] + " n'est pas bâillonné.");
			return;
		}
		...
		if (sender instanceof Player) {
			...
			if (offlinePlayer.getAsPunished().isMute())
				muters.add(offlinePlayer.getAsPunished().getMuter());
			if (offlinePlayer.getIpAsPunished().isMute())
				muters.add(offlinePlayer.getIpAsPunished().getMuter());
			for (String muterName : muters) {
				OfflinePlayer muter = ladder.getOfflinePlayer(muterName);
				int muterLevel = muter.getName().equals("CONSOLE") ? 30 : 0;
				if (muter.hasPlayed())
					muterLevel = ModUtils.getLevel(muter);
				int unmuterLevel = ModUtils.getLevel(unmuter);
				// Rank level
				if (unmuterLevel != 100 && muterLevel >= unmuterLevel
						&& !unmuter.getName().equalsIgnoreCase(muter.getName())) {
					sender.sendMessage("§c➤ Vous n'avez pas le pouvoir de débâillonner ce joueur !");
					return;
				}
			}
		}
		...
		BadblockDatabase.getInstance().addRequest(new Request(
				"INSERT INTO sanctions(pseudo, ip, type, expire, timestamp, date, reason, banner, fromIp) " + "VALUES('"
						...
						+ "', 'unmute', '-1', '" + System.currentTimeMillis() + "', '"
						...
						RequestType.SETTER));
		if (ladder.getPlayer(args[0]) != null) {
