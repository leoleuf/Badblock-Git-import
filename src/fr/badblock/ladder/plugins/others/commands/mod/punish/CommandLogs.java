	...
	public void run(CommandSender sender, String[] args) {
		...
		if (sender instanceof Player) {
			...
			if (bannerLevel != 100 && bannerLevel <= bannedLevel
					&& !banner.getName().equalsIgnoreCase(ofPlayer.getName())) {
				sender.sendMessage(
						"§c➤ Ce joueur est plus haut gradé/au même grade que vous, impossible de voir ses logs !");
				return;
			}
		}
		...
		new Thread() {
			...
			public void run() {
				...
				BadblockDatabase.getInstance().addRequest(new Request(
						"SELECT COUNT(*) AS count FROM sanctions WHERE banner = '"
								+ BadblockDatabase.getInstance().mysql_real_escape_string(ofPlayer.getName()) + "';",
						RequestType.GETTER) {
					@Override
					public void done(ResultSet resultSet) {
						...
						try {
							...
							resultSet.close();
							BadblockDatabase.getInstance().addRequest(new Request(
									"SELECT * FROM sanctions WHERE banner = '"
											+ BadblockDatabase.getInstance()
													.mysql_real_escape_string(ofPlayer.getName())
											+ "' ORDER BY id DESC LIMIT " + firstEntry + "," + limitPerPage + ";",
									RequestType.GETTER) {
								@Override
								public void done(ResultSet resultSet) {
									try {
										...
										while (resultSet.next()) {
											if (first) {
												first = false;
												sender.sendMessage(
														"§7➤ §bSanctions §7effectués par §b" + ofPlayer.getName()
																+ " (page " + nbPage + "/" + totalPages + ") §7:");
												Thread.sleep(100);
											}
											...
											if (type.equals("muteip")) {
												...
												if ((expire > System.currentTimeMillis() || expire == -1)
														&& reason
																.equals(offlinePlayer.getIpAsPunished().getMuteReason())
											...
											if (active.equals("§aON") && type.equals("mute"))
												time = offlinePlayer.getAsPunished().buildMuteTime();
											else if (active.equals("§aON") && type.equals("muteip"))
												time = offlinePlayer.getIpAsPunished().buildMuteTime();
											else if (active.equals("§aON") && type.equals("tempban"))
												time = offlinePlayer.getAsPunished().buildBanTime();
											else if (active.equals("§aON") && type.equals("tempbanip"))
												time = offlinePlayer.getIpAsPunished().buildBanTime();
											...
											sender.sendMessage("§b➤ §8[" + active + "§8] §7" + sanction + " — §b"
													...
													+ banner + (!time.equals("?") ? " §7(§b" + time + "§7)" : "")
													+ " [§b" + date + "§7]");
											Thread.sleep(100);
										}
										if (first) {
											sender.sendMessage(
													"§c➤ " + ofPlayer.getName() + " n'a aucune log sur cette page.");
										}
									} catch (Exception error) {
