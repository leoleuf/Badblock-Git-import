...
public class CommandTempban extends SanctionCommand {

	public static CommandTempban instance;
	 private static final SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy à HH:mm:ss");

	public CommandTempban() {
		super("tempban", "ladder.command.tempban", "tb");
		instance = this;
	}
	...
	public void run(CommandSender sender, String[] args) {
		...
		if (args.length < 3 && !console) {
			sender.sendMessage("§cUsage: /tempban <pseudo> <temps> <raison>");
			return;
		...
		if (sender instanceof Player) {
			...
			if (bannerLevel != 100 && bannerLevel <= bannedLevel) {
				sender.sendMessage(
						"§c➤ Ce joueur est plus haut gradé/au même grade que vous, impossible de le bannir temporairement !");
				return;
			}
		}
		...
		if (time > Time.DAY.convert(60L, Time.MILLIS_SECOND) && !sender.hasPermission("ladder.punish.bypass")) {
			sender.sendMessage("§c➤ Vous ne pouvez pas bannir plus de soixante jours.");
			return;
		...
		long o = time + System.currentTimeMillis();
		// Punish player name
