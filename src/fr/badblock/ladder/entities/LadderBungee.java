...
public class LadderBungee extends ConsoleCommandSender implements BungeeCord, PacketHandler {
	...
	public void handle(PacketPlayerLogin packet) {
		...
		if(plo != null && plo.getBungeeServer() != null) {
			player.disconnect(ChatColor.RED + "Tu es déjà connecté sur BadBlock.");
			return;
		}
		...
		if(event.isCancelled()) {
			...
			sendPacket(new PacketPlayerNickSet(player.getName(), player.getUniqueId(), CommonFilter.filterNames(player.getNickName())));
