...
public class GameTeam implements BadblockTeam {
	...
	public boolean joinTeam(BadblockPlayer player, JoinReason reason) {
		if(!GameAPI.getAPI().getGameServer().isJoinableWhenRunning() && GameAPI.getAPI().getGameServer().getGameState() != GameState.WAITING) return false; // la partie a commenc�e, ouste !
