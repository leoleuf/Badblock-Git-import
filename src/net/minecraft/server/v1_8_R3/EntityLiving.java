...
import fr.badblock.minecraftserver.CustomKB;
...
public abstract class EntityLiving extends Entity {
	...
	public void K() {
		...
		if (this.isAlive()) {
			if (this.inBlock()) {
				...
				if (d0 < 0.0D) {
					this.damageEntity(DamageSource.STUCK, Math.max(1, MathHelper.floor(-d0 * this.world.getWorldBorder().getDamageAmount())));
				}
			}
		}
	...
	public void setHealth(float f) {
		...
		if (this instanceof EntityPlayer) {
			org.bukkit.craftbukkit.v1_8_R3.entity.CraftPlayer player = ((EntityPlayer) this).getBukkitEntity();
			// Squeeze
			if (f < 0.0F) {
	...
	public void a(Entity entity, float f, double d0, double d1) {
		if (this.random.nextDouble() >= this.getAttributeInstance(GenericAttributes.c).getValue()) {
			...
			this.motX /= CustomKB.knockbackFriction;
			this.motY /= CustomKB.knockbackFriction;
			this.motZ /= CustomKB.knockbackFriction;

			this.motX -= d0 / magnitude * CustomKB.knockbackHorizontal;
			this.motY += CustomKB.knockbackVertical;
			this.motZ -= d1 / magnitude * CustomKB.knockbackHorizontal;

			if (this.motY > CustomKB.knockbackVerticalLimit) {
				this.motY = CustomKB.knockbackVerticalLimit;
			}
			// Kohi end
	...
	public void t_() {
		...
		if (f > 0.0025000002F) {
			...
			f1 = (float) org.bukkit.craftbukkit.v1_8_R3.TrigMath.atan2(d1, d0) * 180.0F / 3.1415927F - 90.0F;
		}
	...
	public boolean hasLineOfSight(Entity entity) {
		return this.world.rayTrace(new Vec3D(this.locX, this.locY + this.getHeadHeight(), this.locZ), new Vec3D(entity.locX, entity.locY + entity.getHeadHeight(), entity.locZ)) == null;
	}
