...
public class BlockTNT extends Block {
	...
	public void wasExploded(World world, BlockPosition blockposition, Explosion explosion) {
        if (!world.isClientSide) {
            ...
            EntityTNTPrimed entitytntprimed = new EntityTNTPrimed(loc, world, blockposition.getX() + 0.5F, y, blockposition.getZ() + 0.5F, explosion.getSource()); // PaperSpigot - add loc
            // PaperSpigot end
    ...
    public void a(World world, BlockPosition blockposition, IBlockData iblockdata, EntityLiving entityliving) {
        if (!world.isClientSide) {
            if (iblockdata.get(BlockTNT.EXPLODE).booleanValue()) {
                ...
                double y = blockposition.getY();
                if (!world.paperSpigotConfig.fixCannons) y += 0.5;
                EntityTNTPrimed entitytntprimed = new EntityTNTPrimed(loc, world, blockposition.getX() + 0.5F, y, blockposition.getZ() + 0.5F, entityliving); // PaperSpigot - add loc
                // PaperSpigot end
	...
	public void a(World world, BlockPosition blockposition, IBlockData iblockdata, Entity entity) {
        if (!world.isClientSide && entity instanceof EntityArrow) {
            ...
            if (entityarrow.isBurning()) {
                // CraftBukkit start
                if (org.bukkit.craftbukkit.v1_8_R3.event.CraftEventFactory.callEntityChangeBlockEvent(entityarrow, blockposition.getX(), blockposition.getY(), blockposition.getZ(), Blocks.AIR, 0).isCancelled()) {
                    return;
                }
                // CraftBukkit end
