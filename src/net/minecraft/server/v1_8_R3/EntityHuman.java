...
public abstract class EntityHuman extends EntityLiving {
	...
	public void m() {
		...
		float f1 = (float) ( org.bukkit.craftbukkit.v1_8_R3.TrigMath.atan(-this.motY * 0.20000000298023224D) * 15.0D); // CraftBukkit
				...
				if (f > 0.0F || f1 > 0.0F) {
					...
					if (flag2) {
						if (i > 0) {
							...
							entity.g(
									(double) (-MathHelper.sin(this.yaw * 3.1415927F / 180.0F) * (float) i * CustomKB.knockbackExtraHorizontal),
									CustomKB.knockbackExtraVertical,
									(double) (MathHelper.cos(this.yaw * 3.1415927F / 180.0F) * (float) i * CustomKB.knockbackExtraHorizontal));
							// Kohi end
							this.motX *= 0.6D;
							this.motZ *= 0.6D;
		...
		if (this.vehicle == null) {
			...
			if (this.a(Material.WATER)) {
				...
				if (i > 0) {
					...
					this.applyExhaustion(world.paperSpigotConfig.playerSwimmingExhaustion * i * 0.01F); // PaperSpigot - Configurable swimming exhaustion
				}
			} else if (this.V()) {
				i = Math.round(MathHelper.sqrt(d0 * d0 + d2 * d2) * 100.0F);
				if (i > 0) {
					this.a(StatisticList.l, i);
					this.applyExhaustion(world.paperSpigotConfig.playerSwimmingExhaustion * i * 0.01F); // PaperSpigot - Configurable swimming (diving) exhaustion
				}
			} else if (this.k_()) {
				if (d1 > 0.0D) {
