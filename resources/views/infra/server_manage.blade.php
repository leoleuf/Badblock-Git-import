    ...
    <div class="row">
        <div class="col-12">
            <div class="card-box">
                ...
                <div class="">
                    @foreach($data as $val)
                        <h5>Maintenance : @if($val['maintenance']['state']) <strong
                                    class="text text-success">Actif</strong> @else <strong class="text text-danger">Désactivé</strong> @endif
                        </h5>
                        <pre class='code code-html'><label>MOTD</label><code><input value="{{ $val['description'] }}"
                                                                                    id="motd"></code></pre>
                        <h5>Joueurs en ligne : <strong>{{ $val['players']['online'] }}
                                /{{ $val['players']['max'] }}</strong></h5>
                    @endforeach
                </div>
